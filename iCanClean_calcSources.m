...
function [A,B,R,U,V,STATS] = iCanClean_calcSources(X,Y)
%Calculate sources by performing canonical correlation analysis
%   Nothing fancy going on. Just CCA here plus suppressing some warning messages for < full rank data.

%% convert to double (for rank computation)
X = double(X); Y = double(Y);

%% CCA find sources
  warning('OFF', 'stats:canoncorr:NotFullRank') %prevent warning from CCA for not full rank data
  [A,B,R,U,V,STATS] = canoncorr(X, Y );    %default matlab option
%   [~,LASTID] = lastwarn; disp(LASTID)
%   warning('OFF', LASTID);
